...
os.environ['WANDB_CACHE_DIR'] = '/share/nas2/dmohan/RadioGalaxies-BNNs/radiogalaxies_bnns/wandb_cache'
os.environ['WANDB_ARTIFACTS_DIR'] = '/share/nas2/dmohan/RadioGalaxies-BNNs/radiogalaxies_bnns/wandb_cache'
os.environ['WANDB_DATA_DIR'] = '/share/nas2/dmohan/RadioGalaxies-BNNs/radiogalaxies_bnns/wandb_cache'
...
h0 = 0.5
ess = 584 * 100
train_samples = 10 #mc_samples

wandb_name = 'iVON shuffle'+ str(jobid)
wandb.init(
...
scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0 = 5, T_mult=1, eta_min=0.0, last_epoch=-1)
...
for epoch in range(epochs):
    ...
    avg_loss_train = ivon_utils.train(model, optimizer, criterion, train_loader, device, train_samples)

    

    avg_loss_val, avg_error_val = ivon_utils.validate(model, optimizer, criterion, validation_loader, device, train_samples)
...
test_error = ivon_utils.eval(model, optimizer, test_loader, device, train_samples)
# sampled_probs = eval(test_loader, model, loss_fn, test_mc0)
