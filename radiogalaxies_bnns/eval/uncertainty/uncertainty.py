...
def calibration(path, errors, uncertainties, n_bins, x_label):
    ...
    errors = (pred_label != true_label).astype('uint8')
    #print(np.sum(errors)*100/104)
    ...
    print(entropy, samples_iter)
    
    #normalised entropy of a single pass
    uncertainties = -(class_0 * np.log(class_0) + class_1 * np.log(class_1))/np.log(2)
    #print(uncertainties)
